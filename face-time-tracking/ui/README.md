...
* **Снимок не сохраняется.** Кадр из `<video>` кодируется в JPEG и уходит в запрос; на диск ничего не пишется.
...
* **Офлайн-очередь.** Если сети нет, отметка с временем снимка (`captured_at`) остаётся в `localStorage` и отправляется автоматически при восстановлении связи (повтор каждые 20 секунд и по событию `online`). Снимок хранится локально только до успешной отправки; глубина очереди — 50 отметок. Окно приёма «задним числом» задаётся в воркфлоу 02 (`maxBackdateHours`, по умолчанию 24 часа) — при большей задержке событие запишется текущим временем и будет помечено в метаданных.
...
* **Хранения персональных данных в браузере** — кроме настроек терминала и токена, страницы ничего не сохраняют.
