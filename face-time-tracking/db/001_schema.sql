...
--   * Скрипт идемпотентен: можно запускать повторно, в том числе поверх
--     применённого модуля календаря и отсутствий (003) — тот расширяет часть
--     функций, поэтому одноимённые сносятся перед определением (см. ниже).
-- =============================================================================
...
-- Модуль календаря и отсутствий (003) переопределяет часть функций: добавляет
-- fn_clock и fn_upsert_employee по параметру, а fn_daily_summary и fn_timesheet
-- расширяет колонками результата. Поэтому повторный прогон этого файла с
-- CREATE OR REPLACE либо падает на смене типа возврата, либо, что хуже, молча
-- оставляет рядом устаревшую перегрузку с прежним числом аргументов. Сносим
-- все одноимённые функции заранее: файл целиком в транзакции, снаружи разрыва не видно.
DO $$
DECLARE
    v_sig text;
BEGIN
    FOR v_sig IN
        SELECT p.oid::regprocedure::text
         ...
         WHERE n.nspname = 'timetrack'
           AND p.proname IN ('fn_upsert_employee', 'fn_clock', 'fn_daily_summary', 'fn_timesheet')
    LOOP
